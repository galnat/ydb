...
После создания Pull Request будет запущена [предварительная проверка](#precommit_checks), которая проверяет, что:
  * описание к Pull Request написано,
  * поле Changelog Category содержит одну категорию,
  * поле Changelog Entry содержит не менее 20 символов, если у категории есть [раздел в списке изменений](https://ydb.tech/docs/ru/contributor/suggest-change.md#choose_category).

Регулярно в `main` и `stable` ветках будет запущен процесс, который:
  * собирает Changelog Entry из Pull Request, которые:
    * замержены в ветку,
    * относятся к категории, у которой есть [раздел в списке изменений](https://ydb.tech/docs/ru/contributor/suggest-change.md#choose_category),
    * успешно прошли предварительную проверку;
  * формирует Pull Request на добавление их в CHANGELOG.md.

На ревью этого Pull Request проверят, что выбрана верная категория и описание соответствует [требованиям](https://ydb.tech/docs/en/contributor/suggest-change.md#create_pr_desc). Если описание не будет отвечать требованиям, то автора Pull Request попросят исправить сообщение. После ревью сообщение будет опубликовано в CHANGELOG.md. Перед публикацией новой версии будет собрана история изменений на основании CHANGELOG.md и опубликована в [документацию](../../changelog-server.md).

#### Где посмотреть CHANGELOG.md

В `main` и `stable` ветках в корне лежит файл CHANGELOG.md. В `main` он содержит изменения для всех выпущенных версий и изменения, которые попадут в следующий релиз. В `stable` изменения, которые были в `main` на момент отщепления, и изменения, которые были домержены.

#### Как изменить сообщение в CHANGELOG.md

Если PR еще не был замержен, то можно изменить блок Changelog Entry в описании PR. Если PR был замержен, то следует создать PR на изменение CHANGELOG.md во всех ветках с этим изменением.
